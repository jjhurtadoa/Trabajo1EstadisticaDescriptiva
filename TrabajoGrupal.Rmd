...
### Se cambian los valores a frecuencia relativa para posteriormente graficar los valores.
```{r}
...
```{r}
boxplot(enc$Q05_Saber11, main = "Distribución Puntaje Saber 11",
        outline = FALSE)
```
...
```{r}
boxplot(enc$Q06_PercentilSaber11, main = "Distribución Percentil Pruebas Saber 11",
        outline = FALSE)
```
...
```{r}
barplot(table(enc$Q09_EnAreaMetropolitana))
```
...
```
```{r}
barplot(table(enc$Q23_CambioClimatico))
```
...
#### Se hace una visualización del comportamiento del PAPA con respecto a las carreras registradas en la base de datos, para verificar si tal como pensamos los estudiantes de Estadistica por ser semi-primíparos en la universidad tienen un mayor PAPA.
```{r}
